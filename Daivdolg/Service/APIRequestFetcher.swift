...
class APIRequestFetcher {
  ...
  func fetchConvertedCurrency(amount: String, from: String, to: String, completionHandler: @escaping (Result<ConvertedCurrency, NetworkError>) -> Void) {
    ...
    let url = "https://currency13.p.rapidapi.com/convert/\(amount)/\(from)/\(to)?rapidapi-key=1e7714be28mshb5433aebbd095fap16bed0jsna54da8ce8d86"
    AF.request(url).validate().responseJSON { response in
      ...
      do {
        guard let data = response.data else {
          completionHandler(.failure(.decodingError))
          return }
        let convertedCurrency = try decoder.decode(ConvertedCurrency.self, from: data)
        completionHandler(.success(convertedCurrency))
      } catch {
        print()
        completionHandler(.failure(.badURL))
      }
